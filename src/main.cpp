...
int main(int argc, char *argv[]) {
  ...
  ABM *abm = new ABM(
      edgelist, nodelist, out_degree_bag, recency_table, recency_bins, alpha,
      minimum_alpha, use_alpha, start_from_checkpoint, planted_nodes,
      fully_random_citations, preferential_weight, fitness_weight,
      num_authors_weight, author_reputation_weight, fitness_value_min,
      fitness_value_max, fitness_lag_duration_min, fitness_lag_duration_max,
      fitness_peak_duration_min, fitness_peak_duration_max,
      minimum_preferential_weight, minimum_fitness_weight, in_degree_threshold,
      fitness_threshold, recency_threshold, non_random_generator_probability,
      growth_rate, num_cycles, same_year_citations, neighborhood_sample,
      num_authors_bag, author_max_lifetime, cartel_outdegree_proportion,
      null_cartel, output_file, clonal_cartel_agent_file,
      auxiliary_information_file, log_file, num_processors, log_level);
  abm->main();
  delete abm;
}
