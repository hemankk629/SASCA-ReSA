...
void ABM::ReadPlantedNodes() {
  ...
  std::unordered_map<int, std::string> index_to_header_map =
      Utils::GetIndexToHeaderMap(delimiter, this->planted_nodes);
  std::unordered_map<std::string, int> header_to_index_map =
      Utils::GetHeaderToIndexMap(delimiter, this->planted_nodes);
    ...
    if (line_no > 1) {
      ...
      for (size_t i = 0; i < current_line.size(); i++) {
        if ((int)i != year_header_index) {
          this->planted_nodes_map[current_year][line_no]
                                 [index_to_header_map[i]] = current_line[i];
        }
      }
    }
    line_no += 1;
...
void ABM::FillAuthorReputationArr(
    Graph *graph, const std::unordered_map<int, int> &continuous_node_mapping,
    int *author_reputation_arr) {
  ...
  for (auto const &node : graph->GetNodeSet()) {
    ...
    int continuous_id = continuous_node_mapping.at(node);
    author_reputation_arr[continuous_id] =
...
std::unordered_map<int, int>
ABM::PlantNodes(Graph *graph, double *pa_weight_arr, double *fit_weight_arr,
                double *num_authors_weight_arr,
                double *author_reputation_weight_arr, int *out_degree_arr,
                double *alpha_arr, int *fitness_lag_duration_arr,
                int *fitness_peak_value_arr, int *fitness_peak_duration_arr,
                int *num_authors_arr, int *planted_author_id_arr) {
  ...
  const std::unordered_map<std::string, std::pair<std::string, void *>>
      column_header_to_type_arr_map = {
          ...
          {"author_reputation_weight",
           {"double", (void *)author_reputation_weight_arr}},
          ...
          {"num_authors",
           {"int", (void *)(num_authors_arr + initial_graph_size)}},
          {"author_id", {"int", (void *)(planted_author_id_arr)}}};
  ...
  for (int current_relative_year = 0;
    ...
    if (this->planted_nodes_map.count(current_relative_year)) {
      ...
      for (auto const &[line_no, line_map] : current_year_map) {
        ...
        planted_nodes_line_number_map[chosen_agent_index - initial_graph_size] =
            line_no;
        for (auto const &[planted_feature_name, planted_feature_value] :
             line_map) {
          if (column_header_to_type_arr_map.contains(planted_feature_name)) {
            std::string current_variable_type =
                column_header_to_type_arr_map.at(planted_feature_name).first;
            void *current_variable_arr =
                column_header_to_type_arr_map.at(planted_feature_name).second;
            if (current_variable_type == "double") {
              ((double *)current_variable_arr)[chosen_agent_index -
                                               initial_graph_size] =
                  std::stod(planted_feature_value);
            } else if (current_variable_type == "int") {
              ((int *)current_variable_arr)[chosen_agent_index -
                                            initial_graph_size] =
...
void ABM::FillFitnessArr(
    Graph *graph, const std::unordered_map<int, int> &continuous_node_mapping,
    int current_year, int *fitness_arr) {
  for (auto const &node : graph->GetNodeSet()) {
...
void ABM::PopulateWeightArrs(double *pa_weight_arr, double *fit_weight_arr,
                             double *num_authors_weight_arr,
                             double *author_reputation_weight_arr, int len) {
  pcg_extras::seed_seq_from<std::random_device> rand_dev;
  pcg32 generator(rand_dev);
  if (this->preferential_weight != -1 && this->fitness_weight != -1 &&
      this->num_authors_weight != -1 && this->author_reputation_weight != -1) {
    for (int i = 0; i < len; i++) {
...
void ABM::PopulateFitnessArrs(int *fitness_lag_duration_arr,
                              int *fitness_peak_value_arr,
                              int *fitness_peak_duration_arr, int len) {
  pcg_extras::seed_seq_from<std::random_device> rand_dev;
  pcg32 generator(rand_dev);
  for (int i = 0; i < len; i++) {
    fitness_lag_duration_arr[i] =
...
void ABM::PopulateOutDegreeArr(int *out_degree_arr, int len) {
  std::uniform_int_distribution<int> outdegree_index_uniform_distribution{
      0, (int)(this->out_degree_bag_vec.size() - 1)};
  pcg_extras::seed_seq_from<std::random_device> rand_dev;
  pcg32 generator(rand_dev);
  for (int i = 0; i < len; i++) {
...
void ABM::UpdateGraphAttributesWeights(Graph *graph, int next_node_id,
                                       double *pa_weight_arr,
                                       double *fit_weight_arr,
                                       double *num_authors_weight_arr,
                                       double *author_reputation_weight_arr,
                                       int len) {
  for (int i = 0; i < len; i++) {
    ...
    graph->SetDoubleAttribute("num_authors_weight", current_node_id,
                              num_authors_weight_arr[i]);
    graph->SetDoubleAttribute("author_reputation_weight", current_node_id,
                              author_reputation_weight_arr[i]);
  }
}
...
void ABM::UpdateGraphAttributesNumAuthors(
  ...
  for (auto const &node_id : graph->GetNodeSet()) {
    ...
    graph->SetIntAttribute("num_authors", node_id,
                           num_authors_arr[continuous_id]);
  }
}
...
void ABM::UpdateGraphAttributesInitialAuthorReputations(
  ...
  for (size_t i = 0; i < new_nodes_vec.size(); i++) {
    ...
    graph->SetIntAttribute("initial_author_reputation", current_node_id,
...
void ABM::UpdateGraphAttributesFitnesses(
    ...
    int *fitness_lag_duration_arr, int *fitness_peak_value_arr,
    int *fitness_peak_duration_arr, int initial_graph_size) {
  for (size_t i = 0; i < new_nodes_vec.size(); i++) {
    int current_node_id = new_nodes_vec.at(i);
    int current_weight_arr_index =
        continuous_node_mapping.at(current_node_id) - initial_graph_size;
    graph->SetIntAttribute("fitness_lag_duration", current_node_id,
                           fitness_lag_duration_arr[current_weight_arr_index]);
    graph->SetIntAttribute("fitness_peak_value", current_node_id,
                           fitness_peak_value_arr[current_weight_arr_index]);
    graph->SetIntAttribute("fitness_peak_duration", current_node_id,
                           fitness_peak_duration_arr[current_weight_arr_index]);
  }
}
...
void ABM::UpdateGraphAttributesOutDegrees(Graph *graph, int next_node_id,
                                          int *out_degree_arr, int len) {
  for (int i = 0; i < len; i++) {
    int current_node_id = next_node_id + i;
    graph->SetIntAttribute("assigned_out_degree", current_node_id,
                           out_degree_arr[i]);
  }
}
...
std::vector<int> ABM::GetGraphAttributesGeneratorNodes(Graph *graph,
  ...
  std::string generator_node_string =
      graph->GetStringAttribute("generator_node_string", new_node);
...
void ABM::UpdateGraphAttributesGeneratorNodes(
  ...
  graph->SetStringAttribute("generator_node_string", new_node,
                            generator_node_string);
}

void ABM::CalculateTanhScores(std::unordered_map<int, double> &cached_results,
                              int *src_arr, double *dst_arr, int len) {
  double sum = 0;
#pragma omp parallel for reduction(+ : sum)
  for (int i = 0; i < len; i++) {
    ...
    if (src_arr[i] < 10000) {
      ...
      current_dst = this->peak_constant *
                    std::tanh((pow(src_arr[i], 3) / this->delay_constant) *
                              (1 / this->peak_constant));
    }
...
void ABM::CalculateExpScores(std::unordered_map<int, double> &cached_results,
                             int *src_arr, double *dst_arr, int len) {
  double sum = 0;
#pragma omp parallel for reduction(+ : sum)
  for (int i = 0; i < len; i++) {
...
int ABM::MakeSameYearCitations(
    ...
    int *citations, int current_graph_size) {
  pcg_extras::seed_seq_from<std::random_device> rand_dev;
  pcg32 generator(rand_dev);
  ...
  citations[0] =
      reverse_continuous_node_mapping.at(current_graph_size + current_citation);
  return 1;
}

int ABM::MakeUniformRandomCitationsFromGraph(
    Graph *graph,
    const std::unordered_map<int, int> &reverse_continuous_node_mapping,
    std::vector<int> &generator_nodes, int *citations, int num_cited_so_far,
    int num_citations) {
  if (num_citations <= 0) {
    return 0;
  }
  ...
  if ((int)graph->GetNodeSet().size() - (int)selected.size() <= num_citations) {
    ...
    while (current_citation_index < actual_num_cited) {
      ...
      int current_cited_node =
          reverse_continuous_node_mapping.at(current_citation);
...
int ABM::MakeUniformRandomCitations(
    Graph *graph, const std::unordered_map<int, int> &continuous_node_mapping,
    int current_year, const std::vector<int> &candidate_nodes, int *citations,
    int current_graph_size, int num_citations) {
  if (num_citations <= 0) {
    return 0;
  }
...
int ABM::MakeNullCartelCitations(
    Graph *graph, const std::vector<int> &generator_nodes, int author_id,
    const std::unordered_map<int, int> &continuous_node_mapping,
    const std::unordered_map<int, std::vector<int>> &n_hop_map, int *citations,
...
int ABM::MakeScoredCartelCitations(
    Graph *graph, const std::vector<int> &generator_nodes, int author_id,
    const std::unordered_map<int, int> &continuous_node_mapping,
    const std::unordered_map<int, std::vector<int>> &n_hop_map, int *citations,
    int num_cartel_citations, int current_year,
    const std::unordered_map<int, double> &binned_recency_probabilities,
    double *pa_arr, double *fit_arr, double *na_arr, double *ar_arr,
    double pa_weight, double fit_weight, double num_authors_weight,
    double author_reputation_weight, int current_graph_size) {
  ...
  std::unordered_map<int, std::vector<int>> binned_neighborhood =
      this->BinNeighborhood(graph, current_year, in_neighborhood_cartel_nodes);
  std::unordered_map<int, int> outdegree_per_bin_map = this->BinOutdegrees(
      binned_neighborhood, num_cartel_citations, binned_recency_probabilities);
  int num_locally_cited = 0;
  for (int bin_index = 0; bin_index < this->num_bins - 1;
    ...
    num_locally_cited += this->MakeCitations(
        ...
        binned_neighborhood[bin_index], citations + num_locally_cited, pa_arr,
        fit_arr, na_arr, ar_arr, pa_weight, fit_weight, num_authors_weight,
        author_reputation_weight, current_graph_size,
        outdegree_per_bin_map[bin_index]);
  }
  num_locally_cited += this->MakeUniformRandomCitations(
      graph, continuous_node_mapping, current_year,
      binned_neighborhood[this->num_bins - 1], citations + num_locally_cited,
      current_graph_size, outdegree_per_bin_map[this->num_bins - 1]);
  ...
  if (num_locally_cited < num_cartel_citations) {
    ...
    std::unordered_map<int, std::vector<int>> binned_neighborhood =
        this->BinNeighborhood(graph, current_year,
                              outside_neighborhood_cartel_nodes);
    std::unordered_map<int, int> outdegree_per_bin_map =
        this->BinOutdegrees(binned_neighborhood, remaining_num_cartel_citations,
...
int ABM::MakeCartelCitations(
  ...
  return this->MakeScoredCartelCitations(
      ...
      binned_recency_probabilities, pa_arr, fit_arr, na_arr, ar_arr, pa_weight,
      fit_weight, num_authors_weight, author_reputation_weight,
      current_graph_size);
}

int ABM::MakeCitations(
    Graph *graph, const std::unordered_map<int, int> &continuous_node_mapping,
    int current_year, const std::vector<int> &candidate_nodes, int *citations,
    double *pa_arr, double *fit_arr, double *na_arr, double *ar_arr,
    double pa_weight, double fit_weight, double num_authors_weight,
  ...
  Eigen::Vector4d current_weights(pa_weight, fit_weight, num_authors_weight,
                                  author_reputation_weight);
  double pa_sum = 0.0;
  double fit_sum = 0.0;
  double na_sum = 0.0;
  ...
  if (current_cartel_id > 0) {
    for (auto const &[distance, node_vec] : n_hop_map) {
      for (size_t i = 0; i < node_vec.size(); i++) {
        int node_author_id =
            graph->GetIntAttribute("author_id", node_vec.at(i));
        int node_cartel_id = graph->GetCartelID(node_author_id);
...
std::vector<int> ABM::GetEligibleGeneratorNodes(
    ...
    const std::unordered_map<int, int> &reverse_continuous_node_mapping,
    int *in_degree_arr, int *fitness_arr, int in_degree_threshold,
    int fitness_threshold, int start_year, int current_year,
  ...
  if (current_year - start_year <= recency_threshold) {
    for (int i = graph_size - 1; i >= 0; i--) {
      ...
      if ((current_year - graph->GetIntAttribute("year", current_node_id)) >
          recency_threshold) {
      } else {
        in_degree_eligible_generator_nodes.push_back(
            {in_degree_arr[i], current_node_id});
...
std::vector<int> ABM::GetGeneratorNodes(
  ...
  for (int i = 0; i < num_generator_nodes; i++) {
    int continuous_generator_node = generator_uniform_distribution(generator);
    int generator_node =
        reverse_continuous_node_mapping.at(continuous_generator_node);
    generator_nodes.push_back(generator_node);
  }
  return generator_nodes;
}

std::unordered_map<int, int> ABM::BinOutdegrees(
...
ABM::BinNeighborhood(Graph *graph, int current_year,
...
std::unordered_map<int, std::vector<int>>
ABM::GetNeighborhoodMap(Graph *graph, int current_year,
...
std::unordered_map<int, std::vector<int>>
ABM::GetOneAndTwoDistanceNeighborhoods(Graph *graph, int current_year,
...
std::unordered_map<int, std::vector<int>>
ABM::GetNHopNeighborhood(Graph *graph, int current_year,
                         const std::vector<int> &generator_nodes,
                         int num_hops) {
  std::unordered_map<int, std::vector<int>> n_hop_map;
...
void ABM::LogTime(int current_year, std::string label) {
  if (this->log_level == 1) {
    ...
    this->timing_map[current_year].push_back(
        {label, milliseconds_elapsed.count()});
...
std::chrono::time_point<std::chrono::steady_clock> ABM::LocalLogTime(
    ...
    std::string label) {
  std::chrono::time_point<std::chrono::steady_clock> local_current_time =
      std::chrono::steady_clock::now();
  auto milliseconds_elapsed =
      std::chrono::duration_cast<std::chrono::milliseconds>(local_current_time -
                                                            local_prev_time);
...
std::unordered_map<int, int> ABM::GetNumCitationsPerNeighborhood(
...
void ABM::InitilaizeClonalCartelAgentStruct() {
  ...
  std::unordered_map<int, std::string> index_to_header_map =
      Utils::GetIndexToHeaderMap(delimiter, this->clonal_cartel_agent_file);
  std::unordered_map<std::string, int> header_to_index_map =
      Utils::GetHeaderToIndexMap(delimiter, this->clonal_cartel_agent_file);
    ...
    if (line_no > 1) {
      for (size_t i = 0; i < current_line.size(); i++) {
        ...
        if (current_header == "num_authors_weight") {
          this->clonal_cartel_agent.num_authors_weight =
              std::stod(current_line[i]);
        }
        if (current_header == "author_reputation_weight") {
          this->clonal_cartel_agent.author_reputation_weight =
              std::stod(current_line[i]);
        }
        ...
        if (current_header == "fitness_lag_duration") {
          this->clonal_cartel_agent.fitness_lag_duration =
              std::stoi(current_line[i]);
        }
        if (current_header == "fitness_peak_value") {
          this->clonal_cartel_agent.fitness_peak_value =
              std::stoi(current_line[i]);
        }
        if (current_header == "fitness_peak_duration") {
          this->clonal_cartel_agent.fitness_peak_duration =
              std::stoi(current_line[i]);
        }
      }
    }
...
bool ABM::ValidateBinBoundaries() {
  this->WriteToLogFile(std::to_string(this->bin_boundaries.size()) +
...
bool ABM::ValidateArguments() {
  ...
  if (this->clonal_cartel_agent_file == "") {
    ...
    this->WriteToLogFile("clonal agents: " + this->clonal_cartel_agent_file,
                         Log::info);
  }
  ...
  if (this->preferential_weight == -42) {
    this->WriteToLogFile("Required parameter 'preferential_weight' was not "
    ...
    this->WriteToLogFile("preferential_weight: " +
                             std::to_string(this->preferential_weight),
                         Log::info);
  }
  if (this->fitness_weight == -42) {
    this->WriteToLogFile("Required parameter 'fitness_weight' was not found in "
                         "the 'Agent' section",
                         Log::error);
    return false;
  ...
  if (this->num_authors_weight == -42) {
    this->WriteToLogFile("Required parameter 'num_authors_weight' was not "
                         "found in the 'Agent' section",
                         Log::error);
    return false;
    ...
    this->WriteToLogFile("num_authors_weight: " +
                             std::to_string(this->num_authors_weight),
                         Log::info);
  }
  if (this->author_reputation_weight == -42) {
    this->WriteToLogFile("Required parameter 'author_reputation_weight' was "
    ...
    this->WriteToLogFile("author_reputation_weight: " +
  ...
  if (this->use_alpha) {
    if (this->alpha == -42) {
      this->WriteToLogFile("Required parameter 'alpha' was not found in the "
    ...
    if (this->alpha == -42) {
      this->WriteToLogFile("Alpha ignored. Agents will not split the "
                           "neighborhood into 1 and 2 distance neighborhoods",
                           Log::info);
    } else {
      this->WriteToLogFile("'use_alpha' is false but a value of" +
...
int ABM::main() {
  ...
  this->WriteToLogFile(
      "final graph size is " + std::to_string(final_graph_size), Log::info);
  int *in_degree_arr = new int[final_graph_size]; // live updated in-degree
                                                  // array (changes every year)
  int *fitness_arr = new int[final_graph_size];   // live updated fitness array
                                                  // (changes every year)
  ...
  // the first new agent node has index 0 but is actually index
  // initial_graph_size in the continuous mapping
  ...
  double *num_authors_weight_arr =
      new double[final_graph_size - initial_graph_size];
  double *author_reputation_weight_arr =
      new double[final_graph_size - initial_graph_size];
  ...
  int *fitness_lag_duration_arr =
      new int[final_graph_size - initial_graph_size];
  int *fitness_peak_value_arr = new int[final_graph_size - initial_graph_size];
  int *fitness_peak_duration_arr =
      new int[final_graph_size - initial_graph_size];

  int *planted_author_id_arr = new int[final_graph_size - initial_graph_size];
  std::fill(planted_author_id_arr,
            planted_author_id_arr + final_graph_size - initial_graph_size, -1);
  // int* planted_cartel_id_arr = new int[final_graph_size -
  // initial_graph_size];

  this->PopulateWeightArrs(pa_weight_arr, fit_weight_arr,
                           num_authors_weight_arr, author_reputation_weight_arr,
                           final_graph_size - initial_graph_size);
  this->PopulateAlphaArr(alpha_arr, final_graph_size - initial_graph_size);
  this->PopulateOutDegreeArr(out_degree_arr,
                             final_graph_size - initial_graph_size);
  this->PopulateFitnessArrs(fitness_lag_duration_arr, fitness_peak_value_arr,
                            fitness_peak_duration_arr,
                            final_graph_size - initial_graph_size);
  this->PopulateNumAuthorsArr(graph, num_authors_arr, final_graph_size);
  std::unordered_map<int, int> planted_nodes_line_number_map = this->PlantNodes(
      graph, pa_weight_arr, fit_weight_arr, num_authors_weight_arr,
      author_reputation_weight_arr, out_degree_arr, alpha_arr,
      fitness_lag_duration_arr, fitness_peak_value_arr,
      fitness_peak_duration_arr, num_authors_arr, planted_author_id_arr);
  ...
  for (int current_year = start_year;
       current_year < start_year + this->num_cycles; current_year++) {
    this->prev_time = std::chrono::steady_clock::now();
    int current_graph_size = graph->GetNodeSet().size();
    this->WriteToLogFile("current year is: " + std::to_string(current_year) +
                             " and the graph is " +
                             std::to_string(current_graph_size) +
                             " nodes large",
                         Log::info);
    this->FillInDegreeArr(graph, continuous_node_mapping, in_degree_arr);
    this->LogTime(current_year, "Fill in-degree array");
    this->FillFitnessArr(graph, continuous_node_mapping, current_year,
                         fitness_arr);
    this->LogTime(current_year, "Fill fitness array");
    this->CalculateExpScores(exp_cached_results, in_degree_arr, pa_arr,
                             current_graph_size);
    this->LogTime(current_year, "Process in-degree array");
    this->CalculateExpScores(exp_cached_results, fitness_arr, fit_arr,
                             current_graph_size);
    this->LogTime(current_year, "Process fitness array");
    this->CalculateExpScores(exp_cached_results, num_authors_arr, na_arr,
                             current_graph_size);
    this->LogTime(current_year, "Process num authors array");

    /* initialize new nodes */
    int num_new_nodes = std::ceil(current_graph_size * this->growth_rate);
    this->WriteToLogFile("making " + std::to_string(num_new_nodes) +
                             " nodes this year",
                         Log::info);
    ...
    std::vector<int> eligible_generator_nodes = this->GetEligibleGeneratorNodes(
        graph, current_graph_size, reverse_continuous_node_mapping,
        in_degree_arr, fitness_arr, this->in_degree_threshold,
        this->fitness_threshold, start_year, current_year,
    ...
    for (size_t i = 0; i < new_nodes_vec.size(); i++) {
      ...
      int weight_arr_index =
          continuous_node_mapping[new_node] - initial_graph_size;
      int graph_arr_index = continuous_node_mapping[new_node];
      if (planted_nodes_line_number_map.contains(weight_arr_index) &&
          planted_author_id_arr[weight_arr_index] != -1) {
      ...
      } else if (!planted_nodes_line_number_map.contains(weight_arr_index) &&
      ...
      if (graph->GetCartelID(author_id) > 0) {
        std::vector<int> cartel_generator_nodes =
            this->GetCartelGeneratorNodes(graph, author_id);
        std::vector<int> generator_nodes =
            this->GetGeneratorNodesFromSet(cartel_generator_nodes);
        this->UpdateGraphAttributesGeneratorNodes(graph, new_node,
                                                  generator_nodes);
    ...
    this->FillAuthorReputationArr(graph, continuous_node_mapping,
                                  author_reputation_arr);
    this->LogTime(current_year, "Fill author reputation array");
    this->CalculateExpScores(exp_cached_results, author_reputation_arr, ar_arr,
                             current_graph_size);
...
#pragma omp parallel for reduction(custom_merge_vec_int : new_edges_vec)       \
    ...
    for (size_t i = 0; i < new_nodes_vec.size(); i++) {
      ...
      int citations[this->max_out_degree +
                    1]; // out-degree assumed to be max 249 for now
      int new_node = new_nodes_vec[i];
      // continuous_node_mapping = node id -> 0..n but guaranteed 0 .. initial
      // graph size are seed nodes initial graphsize .. n are agent nodes
      int weight_arr_index =
          continuous_node_mapping[new_node] - initial_graph_size;
      ...
      int author_id = graph->GetIntAttribute("author_id", new_node);
      std::vector<int> generator_nodes =
      ...
      std::unordered_map<int, std::vector<int>> n_hop_map =
          this->GetNeighborhoodMap(graph, current_year, generator_nodes,
                                   num_hops);

      int num_cartel_citations_limit = std::round(
          this->cartel_outdegree_proportion * out_degree_arr[weight_arr_index]);
      int num_fully_random_cited_reserved = std::floor(
          this->fully_random_citations *
          out_degree_arr[weight_arr_index]); // e.g., 5% of out-degree. some
                                             // small number
      ...
      if (graph->GetCartelID(author_id) > 0) {
        current_cartel_size =
            graph->GetCartelAuthors(graph->GetCartelID(author_id)).size() -
      ...
      if (same_year_source_nodes.count(i)) {
        num_actually_cited += this->MakeSameYearCitations(
            ...
            reverse_continuous_node_mapping, citations, current_graph_size);
      }
      local_prev_time =
          this->LocalLogTime(local_parallel_stage_time_vec, local_prev_time,
                             "make same year citations");

      num_actually_cited += this->MakeCartelCitations(
          graph, generator_nodes, author_id, continuous_node_mapping, n_hop_map,
          citations + num_actually_cited, num_cartel_citations, current_year,
          binned_recency_probabilities, pa_arr, fit_arr, na_arr, ar_arr,
          pa_weight, fit_weight, num_authors_weight, author_reputation_weight,
          current_graph_size);
      ...
      if (remaining_citation_quota > 0) {
        std::unordered_map<int, int> num_citations_per_neighborhood =
            this->GetNumCitationsPerNeighborhood(
        ...
        for (size_t current_neighborhood_index = 1;
          ...
          for (int bin_index = 0; bin_index < this->num_bins - 1;
            ...
            num_actually_cited += this->MakeCitations(
                graph, continuous_node_mapping, current_year,
                binned_neighborhood[bin_index], citations + num_actually_cited,
                pa_arr, fit_arr, na_arr, ar_arr, pa_weight, fit_weight,
                num_authors_weight, author_reputation_weight,
                current_graph_size, outdegree_per_bin_map[bin_index]);
          }
          num_actually_cited += this->MakeUniformRandomCitations(
              graph, continuous_node_mapping, current_year,
              binned_neighborhood[this->num_bins - 1],
              citations + num_actually_cited, current_graph_size,
              outdegree_per_bin_map[this->num_bins - 1]);
        }
      }
      for (int j = 0; j < num_actually_cited; j++) {
        if (citations[j] < 0) {
          std::cerr << "[checking before uniform random to graph] regular "
                       "citation negative: "
      ...
      int num_fully_random_cited = out_degree_arr[weight_arr_index] -
                                   num_actually_cited -
                                   generator_nodes.size(); // finalized later
      fully_random_citations_map[i] = num_fully_random_cited;
      num_actually_cited += this->MakeUniformRandomCitationsFromGraph(
          graph, reverse_continuous_node_mapping, generator_nodes, citations,
      ...
      local_prev_time = this->LocalLogTime(local_parallel_stage_time_vec,
                                           local_prev_time, "record edges");
    ...
    this->LogTime(current_year, "find neighborhood",
                  per_stage_time_map["retrieve neighborhood"]);
    this->LogTime(current_year, "bin neighborhood",
                  per_stage_time_map["bin neighborhood"]);
    this->LogTime(current_year, "make same year citations",
                  per_stage_time_map["make same year citations"]);
    this->LogTime(current_year, "make neighborhood citations",
                  per_stage_time_map["make neighborhood citations"]);
    this->LogTime(current_year, "make random citations",
                  per_stage_time_map["make random citations"]);
    this->LogTime(current_year, "record edges",
    ...
    for (size_t i = 0; i < new_nodes_vec.size(); i++) {
      int new_node = new_nodes_vec[i];
      graph->SetIntAttribute("sampled_neighborhood_size", new_node,
                             sampled_neighborhood_sizes_map[i]);
      graph->SetIntAttribute("fully_random_citations", new_node,
    ...
    this->UpdateGraphAttributesFitnesses(
        graph, new_nodes_vec, continuous_node_mapping, fitness_lag_duration_arr,
        fitness_peak_value_arr, fitness_peak_duration_arr, initial_graph_size);
  ...
  this->UpdateGraphAttributesWeights(graph, initial_next_node_id, pa_weight_arr,
                                     fit_weight_arr, num_authors_weight_arr,
                                     author_reputation_weight_arr,
                                     final_graph_size - initial_graph_size);
  ...
  this->UpdateGraphAttributesAlphas(graph, initial_next_node_id, alpha_arr,
                                    final_graph_size - initial_graph_size);
  this->WriteToLogFile("updated alphas", Log::info);
  this->UpdateGraphAttributesNumAuthors(graph, continuous_node_mapping,
                                        num_authors_arr);
  this->WriteToLogFile("updated num authors", Log::info);
  this->UpdateGraphAttributesPlantedNodesLineNumbers(
      graph, initial_next_node_id, planted_nodes_line_number_map);
