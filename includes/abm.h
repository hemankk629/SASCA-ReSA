...
public:
  ABM(std::string edgelist, std::string nodelist, std::string out_degree_bag,
      std::string recency_table, std::string recency_bins, double alpha,
      double minimum_alpha, bool use_alpha, bool start_from_checkpoint,
      std::string planted_nodes, double fully_random_citations,
      double preferential_weight, double fitness_weight,
      double num_authors_weight, double author_reputation_weight,
      int fitness_value_min, int fitness_value_max,
      int fitness_lag_duration_min, int fitness_lag_duration_max,
      int fitness_peak_duration_min, int fitness_peak_duration_max,
      double minimum_preferential_weight, double minimum_fitness_weight,
      int in_degree_threshold, int fitness_threshold, int recency_threshold,
      double non_random_generator_probability, double growth_rate,
      int num_cycles, double same_year_citations, int neighborhood_sample,
      std::string num_authors_bag, int author_max_lifetime,
      double cartel_outdegree_proportion, bool null_cartel,
      std::string output_file, std::string clonal_cartel_agent_file,
      std::string auxiliary_information_file, std::string log_file,
      int num_processors, int log_level)
      : edgelist(edgelist), nodelist(nodelist), out_degree_bag(out_degree_bag),
        recency_table(recency_table), recency_bins(recency_bins), alpha(alpha),
        minimum_alpha(minimum_alpha), use_alpha(use_alpha),
        start_from_checkpoint(start_from_checkpoint),
        planted_nodes(planted_nodes),
        fully_random_citations(fully_random_citations),
        preferential_weight(preferential_weight),
        fitness_weight(fitness_weight), num_authors_weight(num_authors_weight),
        author_reputation_weight(author_reputation_weight),
        fitness_value_min(fitness_value_min),
        fitness_value_max(fitness_value_max),
        fitness_lag_duration_min(fitness_lag_duration_min),
        fitness_lag_duration_max(fitness_lag_duration_max),
        fitness_peak_duration_min(fitness_peak_duration_min),
        fitness_peak_duration_max(fitness_peak_duration_max),
        minimum_preferential_weight(minimum_preferential_weight),
        minimum_fitness_weight(minimum_fitness_weight),
        in_degree_threshold(in_degree_threshold),
        fitness_threshold(fitness_threshold),
        recency_threshold(recency_threshold),
        non_random_generator_probability(non_random_generator_probability),
        growth_rate(growth_rate), num_cycles(num_cycles),
        same_year_citations(same_year_citations),
        neighborhood_sample(neighborhood_sample),
        num_authors_bag(num_authors_bag),
        author_max_lifetime(author_max_lifetime),
        cartel_outdegree_proportion(cartel_outdegree_proportion),
        null_cartel(null_cartel), output_file(output_file),
        clonal_cartel_agent_file(clonal_cartel_agent_file),
        auxiliary_information_file(auxiliary_information_file),
    ...
    if (this->log_level == 1) {
      ...
      if (!this->timing_file_handle) {
        std::cerr << "Opening timing file failed" << std::endl;
        std::cerr << "Requested path: " + this->log_file << "_timing"
                  << std::endl;
        exit(1);
      }
  ...
  std::vector<int> GetGraphAttributesGeneratorNodes(Graph *graph,
                                                    int new_node) const;
  std::vector<int> GetNeighborhood(
      Graph *graph, const std::vector<int> &generator_nodes,
      const std::unordered_map<int, int> &reverse_continuous_node_mapping);
  ...
  void PopulateWeightArrs(double *pa_weight_arr, double *fit_weight_arr,
  ...
  int MakeNullCartelCitations(
      ...
      const std::unordered_map<int, int> &continuous_node_mapping,
      const std::unordered_map<int, std::vector<int>> &n_hop_map,
      int *citations, int num_cartel_citations);
  int MakeScoredCartelCitations(
      Graph *graph, const std::vector<int> &generator_nodes, int author_id,
      const std::unordered_map<int, int> &continuous_node_mapping,
      const std::unordered_map<int, std::vector<int>> &n_hop_map,
      int *citations, int num_cartel_citations, int year,
  ...
  void PopulateFitnessArrs(int *fitness_lag_duration_arr,
  ...
  void CalculateTanhScores(std::unordered_map<int, double> &cached_results,
                           int *src_arr, double *dst_arr, int len);
  void CalculateExpScores(std::unordered_map<int, double> &cached_results,
                          int *src_arr, double *dst_arr, int len);
  ...
  int MakeCitations(Graph *graph,
                    ...
                    int *citations, double *pa_arr, double *fit_arr,
                    double *na_arr, double *ar_arr, double pa_weight,
                    double fit_weight, double num_authors_weight,
                    double author_reputation_weight, int current_graph_size,
                    int num_citations);
  ...
  void UpdateGraphAttributesWeights(Graph *graph, int next_node_id,
  ...
  std::unordered_map<int, int>
  PlantNodes(Graph *graph, double *pa_weight_arr, double *fit_weight_arr,
             double *num_authors_weight_arr,
  ...
  bool ValidateArgument(std::string section, std::string argument_name,
    ...
    if (argument_value == default_value) {
      this->WriteToLogFile("Required parameter '" + argument_name +
    ...
    if constexpr (std::is_same_v<T1, std::string>) {
      ...
      this->WriteToLogFile(
          argument_name + ": " + std::to_string(argument_value), Log::info);
    }
    return true;
  }
